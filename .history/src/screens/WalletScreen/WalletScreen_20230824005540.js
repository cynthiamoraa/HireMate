...
const handleUpdateBalance = async () => {
  ...
  // Handle withdrawal
  if (!withdrawn) {
    const newBalanceAfterWithdrawal = parseFloat(balance) - withdrawAmount;

    if (newBalanceAfterWithdrawal >= 0) {
      await saveBalance(newBalanceAfterWithdrawal);
      await fetchBalance();
      setWithdrawn(true);
  ...
  // Handle top-up
    ...
    const newBalanceAfterTopup = parseFloat(balance) + topupAmount;

    await saveBalance(newBalanceAfterTopup);
    await fetchBalance();
...
useEffect(() => {
 ...
 const onWithDrawPressed = async (n) => {
