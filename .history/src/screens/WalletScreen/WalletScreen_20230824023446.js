  ...
  const getAllTransactions = async () => {
    try {
      ...
      const filteredTransactions = getdata.transactions.filter(
        ...
        setTransactions('this is the data',{transactions: filteredTransactions});
      ...
      //   console.log('Withdrawal Amount:', transaction.withdrawalAmount);
      //   console.log('---------------------------');
      // });
    } catch (error) {
          ...
          {Array.isArray(transactions) ? (
            transactions.map(transaction => {
              if (transaction.phone === Phone) {
                return (
                  <View key={transaction._id} className="flex-row mt-5 mx-3">
                    <Text className="mr-12 font-bold text-xl">
                      Transaction ID: {transaction.transactionId}
                    </Text>
                    <Text className="ml-12 text-lg">
                      Transaction Amount: {transaction.transactionAmount}ksh
                    </Text>
                    <Text className="ml-12 text-lg">
                      Withdrawal Amount: {transaction.withdrawalAmount}ksh
                    </Text>
                  </View>
                );
              }
            ...
            // Display a loading or no data message if transactions is not an array
