...
saveBalance();
  ...
  const sendSms = async () => {
    try {
      const token = await AsyncStorage.getItem('jwt');
      const phoneNumber = '...'; // Replace with the recipient's phone number
      const message = '...'; // Replace with the SMS message

      const response = await fetch('http://your-backend-url/api/sms/send-sms', {
        method: 'post',
        headers: {
          'Content-Type': 'application/json',
        },
