...
useEffect(() => {
  ...
  const fetchBalance = async () => {
    try {
      ...
      const balanceWallet = balance1 + balance2;

      if (withdrawAmount <= balanceWallet) {
     
        const finalBalance = balanceWallet - withdrawAmount;
        setBalance(finalBalance);
        if (withdrawAmount > 0){Alert.alert(
          `Withdrawal of ${withdrawAmount} Successful you will receive a confirmation message `,
        );}
      } else {
