...
const WalletScreen = () => {
  ...
  const updateBalance = async newBalance => {
    try {
      const response = await fetch('http://your-backend-url/update-balance', {
        method: 'post',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (response.ok) {
        ...
        console.log('Updating balance failed:', data.error);
    ...
    <SafeAreaView>
      <View className="bg-[#0a0a0a] rounded-b-lg">
        ...
        <View className=" p-12 items-center">
          ...
          <Text className="text-lg  text-white p-2">{calculateTotalAmount()} ksh</Text>
          <CustomButton
            text="choose card"
