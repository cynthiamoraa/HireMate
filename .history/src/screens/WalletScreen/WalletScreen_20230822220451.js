  ...
  const fetchBalance = async () => {
    try {
      ...
      let finalBalance = balance1 + balance2;
        console.log(result);
        console.log(balance1,balance2,withdrawAmount,finalBalance);
     
      if (withdrawAmount > 0 && withdrawAmount <= finalBalance) {
        // Perform withdrawal and update balance
        const newBalance = finalBalance - withdrawAmount;
        ...
        if (withdrawAmount <= 0) {
          Alert.alert('Please enter a valid withdrawal amount.');
        } else {
  ...
  const saveBalance = async (newBalance) => {
    try {
      ...
      await fetch('https://hiremate-hnjv.onrender.com/wallet', {
        ...
        body: JSON.stringify({balance: newBalance}), // Send the new balance
      });
    } catch (error) {
    ...
    <SafeAreaView>
      <View className="bg-[#0a0a0a] rounded-b-lg">
        ...
        <View className=" p-12 items-center">
          ...
          <Text className="text-lg  text-white p-2">{newBalance} ksh</Text>
          {/* <CustomButton
