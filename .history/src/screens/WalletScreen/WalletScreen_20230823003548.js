...
;
    const TopUpAmount = () => {
      ...
      const WithDrawAmount = () => {
        const {params} = route;
        if (params && params.withdraw && params.Wphone) {
...
useEffect(() => {
   ...
   const handleWithdrawal = async () => {
    ...
    if (withdrawAmount > 0) {
      const withdrawalTransaction = {
        _id: Math.random().toString(), // Generate a unique ID (you can adjust this method)
        phone,
        amount: withdrawAmount,
        type: 'withdrawal',
      };

      // Update the transactions state with the new withdrawal
      setTransactions([...transactions, withdrawalTransaction]);

      // Save the updated transactions to local storage
      saveTransactions();

      // Reset the withdrawal flag
      setWithdrawn(false);

      // ... The rest of your withdrawal logic
    } else {
      // Display an alert indicating invalid withdrawal amount
      Alert.alert('Invalid withdrawal amount.');
    }
  };
  ...
  const userWithdrawals = transactions.filter(
    transaction =>
      transaction.phone === Phone && transaction.type === 'withdrawal'
  );
            ...
            if (item.phone === Phone) {
              return (
                <View key={item._id} className="flex-row mt-5 mx-3">
                  ...
                  {/* Check the transaction type and render accordingly */}
                  {item.type === 'withdrawal' && (
                    <Text className="ml-2 text-red-500">Withdrawal</Text>
                  )}
                  {item.type === 'topup' && (
                    <Text className="ml-2 text-green-500">Top-up</Text>
                  )}
                </View>
              );
            }
