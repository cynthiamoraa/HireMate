...
//    const [applicantId, setApplicantId] = useState('');
  ...
  const handlePayment = async () => {
    console.log('hello buttonpresse');
    try {
      const {params} = route;
      if(params && params.applicantId && params.amount && params.userId){
      const token = await AsyncStorage.getItem('jwt');
      const response = await fetch('http://172.18.0.1:8000/sendfinalizedpayment', {
