    ...
    try {
      ...
      if (params && params.applicantId ) {
        ...
        const response = await fetch(
          'https://hiremate-hnjv.onrender.com/review',
          {
            method: 'post',
            headers: {
        ...
        if (response.ok) {
          ...
          Alert.alert('Thank you for your Review! Remember to close the job posting!');

          navigation.navigate('Applicant');
        } else {
          console.log('posting failed:', data.message);
          Alert.alert('please make sure to pick a rating !');
        }
      }
    } catch (error) {
