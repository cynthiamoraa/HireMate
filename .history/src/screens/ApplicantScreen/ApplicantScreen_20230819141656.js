...
const openMail = (email) => {
  ...
  const pressed = async(id)=>{
     const newPressedStates = {...pressedStates, [id]: true}; // Update the applied state for the specific item
...
const review = (id)=>{
  console.log('review');
     ...
     const newPressedStates = {...pressedStates, [id]: false}; // Update the applied state for the specific item
     setPressedStates(newPressedStates);
        ...
        <View>
          ...
          {userData.apply.map((applicant, index) => (
            <View key={index} className="p-3 flex-row">
              ...
              {pressedStates[applicant._id] ? (
                  <CustomButton
                text="review"
                onPress={() => review(applicant._id)}
                bgColor="#3D7DEB"
                textColor={'#fff'}
              />
              ) : (
