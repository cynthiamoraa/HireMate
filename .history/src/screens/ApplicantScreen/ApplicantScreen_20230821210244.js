...
const handlePayment = async (applicantId) => {
  ...
  return (
    <View className="m-4 ">
      {userData && (
        <View>
          <Text className="text-lg font-semibold text-black">This job was posted by</Text>
          <V
          <Text className="text-lg p-1 ml-2 text-black">Title: {userData.title}</Text>
          <Text className="text-lg p-1 ml-2 text-black">
            Username: {userData.postedBy.username}
          </Text>
          <Text className="text-lg p-1 ml-2 text-black">Amount : {userData.amount}ksh</Text>
          <Text className="text-lg p-1 ml-2 text-black">
            Email: {userData.postedBy.email}
          </Text>

          <Text className="text-lg font-semibold text-black">Applicants for the job:</Text>
          {userData.apply.map((applicant, index) => (
            <ScrollView key={index} className="p-3 border rounded-2xl my-2">
              <View>
                <Text className="text-lg text-black">Username: {applicant.username}</Text>
                <Text className="text-lg text-black">Email: {applicant.email}</Text>
              </View>
