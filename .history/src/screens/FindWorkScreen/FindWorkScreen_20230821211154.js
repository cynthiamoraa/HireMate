...
const FindWorkScreen = () => {
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false} className="m-4">
      <Text className="text-center font-bold mt-5 text-lg text-black">FindWork</Text>
      <View className=" flex ">
        <TextInput
          className="border mt-5  p-4 rounded-full text-lg text-black"
          placeholder="Search..."
      ...
      {data.map(item => {
        return (
          <View
            key={item._id}
            className="border mt-10 bg-slate-200 rounded-2xl">
            <Text className="pl-2 font-bold mt-5 text-xl text-black">
              {item.title}
            </Text>
