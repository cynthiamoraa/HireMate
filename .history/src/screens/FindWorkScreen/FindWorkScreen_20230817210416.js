...
const FindWorkScreen = () => {
   ...
   const fetchData = async () => {
    ...
    const handleApply = async (postid,userid) => {
      ...
      try {
        ...
        console.log(result);
        console.log(data1);
        data1.posts.forEach(post => {
          console.log('Apply items for post:', post.title);
          post.apply.forEach((item, index) => {
            console.log(`Item ${index + 1}:`, item);
          });
      });
      console.log(data1.posts[3]);
        const newData = data.map(item => {
          if (item._id === result._id) {
            ...
            if (item._id === postid) {
              return {
                ...item,
                applicants: [...item.applicants, userid], // Add the user ID
              };
            }
            return item;
          }
